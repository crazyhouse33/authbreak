...
// We declare it there cause this function should be static, however if we want to be able to test it in an unit tets, we must export it. So we do it but dont expose it in the interface, stil removing
// the warning
...
int main() {
  Composed_classifier *classifier = composed_classifier_new(true);
  ...
  generate_test_separation("status  != 18", status, not_equal, "18");
  generate_test_separation("status = 18", status, equal, "18");
  generate_test_separation("status < 18", status, inferior_strict, "18");
